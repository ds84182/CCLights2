...
import ds.mods.CCLights2.utils.LuaMethodUtils;
...
public class PacketHandler implements IPacketHandler {
	...
	public void onPacketData(INetworkManager manager,
		...
		else if (packet.channel.equals("NetMethod"))
		{
			String method = dat.readUTF();
			Class<?> c = null;
			try {
				String cn = dat.readUTF();
			...
			int arglen = dat.readInt()-1;
			Object hobj = Serialize.unserialize(dat);
			Object[] args = new Object[arglen];
			for (int i = 0; i<arglen; i++)
			{
				args[i] = Serialize.unserialize(dat);
			}
			try {
				//System.out.println(method);
				LuaMethodUtils.runMethod(c, method, hobj, args, false);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
